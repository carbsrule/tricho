...
abstract class Column implements QueryField, ColumnInterface {
    ...
    static function fromXMLNode (DOMElement $node) {
        ...
        $col = new $attribs['class'] ($attribs['name']);
    ...
    /**
     * Sets a parameter to use for this column.
     * Parameters should be set by methods in subclasses of Column, not manually specified.
     * 
     * Example parameters follow:
     * - <b>size</b>: dimensions of a regular field (width in characters),
     *     textarea (width x height: in characters for plain text; in pixels for rich text fields),
     *     or image (width x height in pixels - specify 0 to have an unrestricted dimension).
     * 
     * - <b>exact_size</b>: if true, the large variant of an image will be forced to match a size specification exactly
     *     (i.e. it will not be resized).
     * - <b>large_crop</b>: if true, the large variant of an image will be cropped to fit in the specified dimensions.
     * - <b>thumb_crop</b>: if true, the small variant of an image will be cropped.
     * - <b>thumbnails</b>: an array of thumbnail sizes to generate when saving an image.
     * - <b>storage_location</b>: where to save uploaded files for file or image columns, relative to ROOT_PATH_FILE.
     * - <b>max_file_size</b>: the maximum file ipload size allowed for an image column.
     * - <b>icon</b>: whether or not a file icon or image thumbnail should be shown on the main view.
     * - <b>allow_del</b>: if true, when editing a record that already has a file saved for a file or image column,
     *     a tickbox will provided that allows for deletion of the existing file.
     * - <b>cache_period</b>: the period for which to cache uploaded files for an image column. Specified as a number
     *     followed by a letter ([m]inutes, [h]ours, or [d]ays), e.g. '7d' for 7 days
     * 
     * - <b>date_min_year</b>: the minimum year allowed for a DATE or DATETIME field (can be hard set, e.g. 2008, or
     *     relative to the current year, e.g. '-1': 1 year ago)
     * - <b>date_max_year</b>: the maximum year allowed for a DATE or DATETIME field
     * - <b>inc_secs</b>: if true, a TIME or DATETIME field will allow input of the number of seconds
     * - <b>date_default</b>: a default date to use for fields with the date option
     * - <b>date_format</b>: how date option fields should be displayed when not providing input fields
     * - <b>text_year</b>: if true, the date input will have a text input instead of a select list for the year portion
     * 
     * - <b>encryption_mechanism</b>: the encryption mechanism to use (md5 or sha1) for a password field
     * - <b>require_existing_password</b>: if true, the user must enter the existing password in addition to a new
     *     password when changing the password for a record
     * 
     * - <b>tinymce_buttons</b>: the list of buttons to display for a richtext (TinyMCE) field. New lines of buttons
     *     are specifed by slashes, and buttons are separated by commas. Buttons that are only accessible by setup
     *     users are to be prefixed with #.
     * 
    ...
    function setParam ($key, $value) {
        ...
        throw new Exception ('setParam is deprecated');
    }
    
    /**
     * Clears a parameter that is stored for this column.
     * 
     * @param string $key The parameter to unset.
     */
    function unsetParam ($key) {
        unset ($this->params[$key]);
        throw new Exception ('unsetParam is deprecated');
    }
    
    /**
     * Clears all parameters that are stored for this column.
     */
    function clearParams () {
        $this->params = array ();
        throw new Exception ('clearParams is deprecated');
    }
    
    /**
     * Gets a parameter to use for this column (see {@link Column::setParam()}).
     *
    ...
    function getParam ($key) {
        throw new Exception ('getParam is deprecated');
        return $this->params[$key];
    }
    
    /**
     * Gets the parameters to use for this column.
     *
     * @return array $params The parameters.
     */
    function getParams () {
        throw new Exception ('getParams is deprecated');
        return $this->params;
    }
