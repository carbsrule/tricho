...
//    $col->setType ($type);
    ...
    /* check which filters were set and add them to the 'filters' param */
    $filter_list = array ('trim', 'tabs', 'multispace', 'nl', 'tags', 'br');
    $filters = array ();
    ...
    // params are deprecated
    /*
    // set filters param (default if none set)
    $filters = implode (', ', $filters);
    if ($filters != '') {
        $col->setParam ('filters', $filters);
    } else {
        $col->setParam ('filters', 'none');
    }
    */
    ...
    case 'image':
        ...
        if ($config['restrict'] == 1) {
            settype ($config['restrict_w'], 'integer');
            settype ($config['restrict_h'], 'integer');
            $col->setParam ('size', "{$config['restrict_w']}x{$config['restrict_h']}");
        }
        
        $thumbnails = array ();
        if ($config['medium'] == 1) {
            settype ($config['med_w'], 'integer');
            settype ($config['med_h'], 'integer');
            $thumbnails['medium'] = "{$config['med_w']}x{$config['med_h']}";
        }
        if ($config['thumb'] == 1) {
            settype ($config['thumb_w'], 'integer');
            settype ($config['thumb_h'], 'integer');
            $thumbnails['small'] = "{$config['thumb_w']}x{$config['thumb_h']}";
        }
        if (count ($existing_thumbnails) > 0) {
            foreach ($existing_thumbnails as $name => $thumb) {
                $thumbnails[$name] = $thumb;
            }
        }
        
        if (count($thumbnails) > 0) {
            $col->setParam ('thumbnails', $thumbnails);
        }
        $col->setParam ('storage_location', $config['storeloc']);
        $col->setParam ('max_file_size', $config['max_file_size']);
        $col->setParam ('icon', $config['image_icon']);
        if ($config['img_del'] == 1) {
            $col->setParam ('allow_del', true);
        } else {
            $col->setParam ('allow_del', false);
        }
        
        if ($config['large_restrict_type'] == 'exact_size') {
            $col->setParam ('exact_size', true);
        } else if ($config['large_restrict_type'] == 'crop') {
            $col->setParam ('large_crop', true);
        }
        
        if ($config['thumb_restrict_type'] == 'crop') {
            $col->setParam ('thumb_crop', true);
        }
        
        // Split and recombine cache period to ensure validity
        $cache_period = $config['cache_length']. $config['cache_scale'];
        $cache_period = implode ('', determine_cache_length ($cache_period));
        $col->setParam ('cache_period', $cache_period);
        break;
    ...
    case 'file':
        $col->setParam ('storage_location', $config['storeloc']);
        $col->setParam ('max_file_size', $config['max_file_size']);
        $col->setParam ('icon', $config['file_icon']);
        if ($config['file_del'] == 1) {
            $col->setParam ('allow_del', true);
        } else {
            $col->setParam ('allow_del', false);
        }
    ...
    case 'date':
        $col->setParam ('date_min_year', $config['year_min']);
        $col->setParam ('date_max_year', $config['year_max']);
        if ($config['sqltype'] == SQL_TYPE_DATETIME or $config['sqltype'] == SQL_TYPE_TIME) {
            $col->setParam ('inc_secs', $config['inc_secs']);
        }
        if ($config['sqltype'] == SQL_TYPE_DATE or $config['sqltype'] == SQL_TYPE_DATETIME) {
            $col->setParam ('text_year', $config['text_year'] == 1);
        }
        
        switch ($config['default_date']) {
        case 'current':
            if ($config['sqltype'] == SQL_TYPE_DATETIME or $config['sqltype'] == SQL_TYPE_TIME) {
                $col->setParam ('date_default', 'NOW+');
            } else {
                $col->setParam ('date_default', 'NOW');
            }
            break;
        ...
        case 'specific':
            if ($config['sqltype'] == SQL_TYPE_DATETIME) {
                $col->setParam ('date_default', tricho_date_build ('specific_date'));
            } else if ($config['sqltype'] == SQL_TYPE_TIME) {
                list ($def_date, $def_time) = explode (' ', (tricho_date_build ('specific_date')));
                $col->setParam ('date_default', $def_time);
            } else {
                list ($def_date, $def_time) = explode (' ', (tricho_date_build ('specific_date')));
                $col->setParam ('date_default', $def_date);
            }
            break;
            
        default:
            $col->setParam ('date_default', false);
        }
        
        // date format
        $format = $config['date_format'];
        if ($format == 'custom') {
            if (trim ($config['custom_date_format']) == '') {
                $_SESSION['setup']['warn'] = 'You specified an invalid custom format, and therefore the date format was reverted to long Australian date';
                $config['custom_date_format'] = '';
            }
            $format = $config['custom_date_format'];
        }
        $col->setParam ('date_format', $format);
        break;
    
    case 'richtext':
        if ($config['tinymce_buttons'][0] != '') {
            $col->setParam ('tinymce_buttons', $config['tinymce_buttons']);
        }
        if ($config['tinymce_tags_allow'] != '') {
            $col->setParam ('tinymce_tags_allow', $config['tinymce_tags_allow']);
        }
        if ($config['tinymce_tags_replace'] != '') {
            $col->setParam ('tinymce_tags_replace', $config['tinymce_tags_replace']);
        }

        if ($config['tinymce_tags_deny'] != '') {
            $col->setParam ('tinymce_tags_deny', $config['tinymce_tags_deny']);
        }
        break;
        
    case 'richtext2':
        if ($config['xstd_toolbar'] != '') $col->setParam ('xstd_toolbar', $config['xstd_toolbar']);
        if ($config['xstd_styles'] != '') $col->setParam ('xstd_styles', $config['xstd_styles']);
        break;
    
    }
    
    if ($config['size'] != null) $col->setParam ('size', $config['size']);
    // options are deprecated
    //$col->setOption ($config['options']);
    $col->setSqlAttributes (implode (' ', $sql_attributes));
    
    $col->setMandatory($config['mandatory']);
    $col->setComment($config['comments']);
    
    $config['onchange'] = trim ($config['onchange']);
    if ($config['onchange'] != '') {
        $col->setParam ('onchange', $config['onchange']);
    }
