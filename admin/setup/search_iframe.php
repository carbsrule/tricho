...
function order_iframe ($table) {
    ...
    foreach ($in_list as $id => $item) {
        ...
        echo '</td><td>';
        if ($i++ > 0) {
            echo '<a href="search_iframe_action.php?sect=in&amp;go=up&amp;id=', $id, '">',
                '<img src="',ROOT_PATH_WEB, IMAGE_ARROW_UP,'" border="0"></a>';
        } else {
            echo '&nbsp;';
        }
        echo '</td><td><a href="search_iframe_action.php?sect=in&amp;go=down&amp;id=', $id, '">',
    ...
    foreach ($out_list as $id => $item) {
        echo '<tr><td>';
        show_item ($item);
        echo '</td><td colspan="2"><a href="search_iframe_action.php?sect=out&amp;go=up&amp;id=', $id, '">',
        '<img src="',ROOT_PATH_WEB, IMAGE_ARROW_UP,'" border="0"></a>';
        echo "</td></tr>\n";
    }
    echo "</table>\n";
...
order_iframe ($_SESSION['setup']['create_table']['table']);
